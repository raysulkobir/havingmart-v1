...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
    ...
    private function process_guest_checkout(Request $request)
    {
        ...
        $carts = ($temp_user_id != null)
            ? Cart::where('temp_user_id', $temp_user_id)->whereIn('id', $cart_ids)->get()
            : collect();
    ...
    private function process_logged_in_checkout(Request $request)
    {
        ...
        if (!empty($cart_ids)) {
            $cart_query->whereIn('id', $cart_ids);
        }
        ...
        foreach ($carts as $cartItem) {
            $cartItem->address_id = $request->address_id;
            $cartItem->shipping_type = 'home_delivery';
            $cartItem->shipping_cost = $shipping_charge;
            $cartItem->save();
        }
    ...
    public function order_confirmed()
    {
        ...
        Session::forget('express_checkout_cart_ids');
