...
class HomeController extends Controller
{
    ...
    public function otp_send(Request $request)
    {
        try {
            $request->validate([
                'phone' => [
                    'required',
                    'regex:/^(013|014|015|016|017|018|019)\d{8}$/'
                ],
            ]);
        ...
        $mobileNumberCheck = User::where('phone', $mobile)->first();
        // $otp = rand(10000, 99999);
        $otp = "1234";
        $message = urlencode("ShahinBazar: {$otp}");
        // smsSender($mobile, $message);

        if ($mobileNumberCheck) {
            $mobileNumberCheck->verification_code = $otp;
            $mobileNumberCheck->save();
        } else {
            User::create([
                'name' => '',
        ...
        return redirect()
            ->route('user.otp_verify')
            ->with('mobile', $request->phone);
    }
    ...
    private function cartUpdate($request){
        $temp_user_id = $request->session()->get('temp_user_id');
        if($temp_user_id != null) {
            Cart::where('temp_user_id', $temp_user_id)->update(['user_id' => Auth::user()->id]);
        }
    }
    ...
    public function cart_login(Request $request)
    {
        ...
        if ($user != null) {
            if (Hash::check($request->password, $user->password)) {
                ...
                // Handle checkout type redirect
                if ($request->checkout_type == 'express') {
    ...
    public function profile(Request $request)
    {
        if (Auth::user()->user_type == 'seller') {
            ...
            $districts = City::get();
            return view('frontend.user.profile', compact('districts'));
        }
    }
    ...
    public function load_trending_products(Request $request)
    {
        ...
        $cacheKey = 'trending_products_ajax_page_' . $page;

        // Cache the result for 10 minutes to protect the database from parallel load
        $data = \Illuminate\Support\Facades\Cache::remember($cacheKey, 600, function() use ($page) {
            $limit = 12;
            $offset = ($page - 1) * $limit;
            $eagerRelations = ['product_translations', 'taxes', 'stocks', 'brand', 'category', 'user'];
            ...
            $has_more = $trending_products->count() > $limit;
            if ($has_more) {
                $trending_products = $trending_products->take($limit);
            }
    ...
    public function product(Request $request, $slug)
    {
        ...
        if ($detailedProduct) 
        {
            ...
            if ($detailedProduct != null && $detailedProduct->published) {
                $product_view_session_key = 'viewed_product_' . $detailedProduct->id;
                if (!$request->session()->has($product_view_session_key)) {
                    Product::where('id', $detailedProduct->id)->increment('product_view_count');
                    $detailedProduct->product_view_count = ((int) $detailedProduct->product_view_count) + 1;
                    $request->session()->put($product_view_session_key, true);
                }
    ...
    function clearCV(Request $request)
    {
        Artisan::call('cache:clear');
        Artisan::call('view:clear');
        flash(translate('Cleared Old Cache & Views'))->success();
        return redirect()->route('home');
    }
}
