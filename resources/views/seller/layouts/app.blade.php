...
<body class="">
		...
		<div class="aiz-content-wrapper">
			...
			<div class="aiz-main-content">
				...
				<div class="bg-white text-center py-3 px-15px px-lg-25px mt-auto border-sm-top">
					<p class="mb-0">&copy; {{ date('Y') }} {{ get_setting('site_name') }}. All Rights Reserved. </p>
				</div>
			</div>
		</div>
	</div>
    ...
    <script type="text/javascript">
        ...
        $("#name").keyup(function(event) {
	        var name = $(this).val();
	        $("#meta_title").val(name);
	        var nameLowerCase = name.toLowerCase().split(' ').join('-');
	        var scCleaned = nameLowerCase.replace('&', 'and');
	        var qCleaned = scCleaned.replace("'", '');
	        var slug = qCleaned.replace(",", '');
	        $("#slug").val(slug);
	    });
    </script>
