...
<div class="card">
    <form class="" id="sort_products" action="" method="GET">
        <div class="card-header row gutters-5">
            ...
            <div class="col-md-2 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="section_type" id="section_type" onchange="sort_products()">
                    <option value="">{{ translate('Types') }}</option>
                    <option value="featured" @isset($section_type) @if($section_type == "featured") selected @endif @endisset">{{translate('Featured')}}</option>
                    <option value="new_product" @isset($section_type) @if($section_type == "new_product") selected @endif @endisset">{{translate('New Products')}}</option>
                    <option value="best_selling" @isset($section_type) @if($section_type == "best_selling") selected @endif @endisset">{{translate('Best Selling')}}</option>
                    <option value="category_by_featured" @isset($section_type) @if($section_type == "category_by_featured") selected @endif @endisset">{{translate('Category By Featured')}}</option>
                </select>
            </div>
            <div class="col-md-2">
            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        @if($type == 'Seller' || $type == 'All')
                            <th data-breakpoints="lg">{{translate('By')}}</th>
                        @endif
                        ...
                        <th data-breakpoints="lg">{{translate('New Products')}}</th>
                        <th data-breakpoints="lg">{{translate('Best Selling')}}</th>
                        <th data-breakpoints="lg">{{translate('Category By Featured')}}</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_featured(this, 'featured')" value="{{ $product->id }}" type="checkbox" <?php if ($product->featured == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        {{-- New Products --}}
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_featured(this, 'new_product')" value="{{ $product->id }}" type="checkbox" <?php if ($product->new_product == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        {{-- Best Selling --}}
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_featured(this, 'best_selling')" value="{{ $product->id }}" type="checkbox" <?php if ($product->best_selling == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        {{-- Category By Featured --}}
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_featured(this, 'category_by_featured')" value="{{ $product->id }}" type="checkbox" <?php if ($product->category_by_featured == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        <td>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_featured(el, type=''){
            if(el.checked){
                var status = 1;
            }
            else{
            ...
            $.post('{{ route('products.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, type:type, status:status}, function(data){
                if(data == 1){
