        ...
        <tbody>
        	...
        	<tr>
				...
				@if($page->type == 'home_page')
        			<td><a target="_blank" href="{{ route('home') }}" class="text-reset">{{ $page->getTranslation('title') }}</a></td>
					<td>{{ route('home') }}</td>
				@else
        			<td><a target="_blank" href="{{ route('custom-pages.show_custom_page', $page->slug) }}" class="text-reset">{{ $page->getTranslation('title') }}</a></td>
